function COST = shortest_path(I,m,n)
%I(:,:,1) %R channel
%I(:,:,2) %G channel
%I(:,:,3) %B channel
[x,y,z] = size(I)
...
C =inf(3);
C(1) = (M-D(1))*sqrt(2)/1.5;
COST(x*(j-1)+i,x*(j-2)+i-1) = C(1);
COST(x*(j-2)+i-1,x*(j-1)+i) = C(1);
C(2) = (M-D(2))/1.5;
COST(x*(j-1)+i,x*(j-2)+i) = C(2);
COST(x*(j-2)+i,x*(j-1)+i) = C(2);
C(3) = (M-D(3))*sqrt(2)/1.5;
COST(x*(j-1)+i,x*(j-2)+i+1) = C(3);
COST(x*(j-2)+i+1,x*(j-1)+i) = C(3);
C(4) = (M-D(4))/1.5;
COST(x*(j-1)+i,x*(j-1)+i-1) = C(4);
COST(x*(j-1)+i-1,x*(j-1)+i) = C(4);
C(6) = (M-D(6))/1.5;
COST(x*(j-1)+i,x*(j-1)+i+1) = C(6);
COST(x*(j-1)+i+1,x*(j-1)+i) = C(6);
C(7) = (M-D(7))*sqrt(2)/1.5;
COST(x*(j-1)+i,x*j+i-1) = C(7);
COST(x*j+i-1,x*(j-1)+i) = C(7);
C(8) = (M-D(8))/1.5;
COST(x*(j-1)+i,x*j+i) = C(8);
COST(x*j+i,x*(j-1)+i) = C(8);
C(9) = (M-D(9))*sqrt(2)/1.5;
COST(x*(j-1)+i,x*j+i+1) = C(9);
COST(x*j+i+1,x*(j-1)+i) = C(9);
